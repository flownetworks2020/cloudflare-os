...
Two precautions surround the abort (`OverseerImpl.scheduleAccessRestart`): the severed edge is flushed with `ctx.storage.sync()` first (because `ctx.abort()` does not respect the output gate, a restart could otherwise come back with the change lost), and the abort is delayed ~100ms so the triggering RPC's response reaches the caller -- typically the owner, who is also connected -- before their own connection drops. The flush is then repeated immediately before the abort, with nothing awaited in between, because the input gate is open across that delay: a *concurrent* access mutation -- a revocation that has written its edge and is still awaiting `tearDownLostObservers` -- would otherwise be aborted with its write still buffered and silently never happen, while its caller was told it succeeded. That final sync is sufficient rather than merely narrower because the input gate closes for the duration of a storage operation ("while a storage operation is executing, no events shall be delivered to the object except for storage completion events"), so nothing can interleave a mutation between it and the abort. Concurrent triggers coalesce onto one timer rather than racing several aborts. The disconnect reaches the browser through the existing `notifyClosed` plumbing: when the Overseer DO aborts, the per-session `notifyClosed` stub is disposed without being called, which `AuthenticatedApiImpl` treats as a lost connection and reacts to by killing the browser WebSocket, forcing a reconnect.
