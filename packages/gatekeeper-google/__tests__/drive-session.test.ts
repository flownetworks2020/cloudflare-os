...
describe("Drive native sessions", () => {
  ...
  it.each([403, 404])(
    "keeps an account-scope %s probe failure private",
    async status => {
      let { session, prepared, authorizations } = core({
        getFile: async () => { throw new DriveApiRequestError(status); },
      ...
      expect(authorizations).toEqual([{
        ...
        description: "Check whether the connected account can access a requested Drive file.",
        excludeObservers: ["observer-1", "observer-2"],
      }]);
    },
  );
