...
function coordinator(kv: CredentialsKv, upgrade?: CredentialCoordinatorOptions<Creds>["upgrade"]) {
  return new CredentialCoordinator<Creds>(kv, { expiresAt: creds => creds.expiresAt, upgrade });
}
...
describe("CredentialCoordinator", () => {
  ...
  it("honours a refresh window wider than the default", async () => {
    ...
    instance.commit({ token: "soon", expiresAt: Date.now() + 3 * 60_000 });
  ...
  it("lets a reconnect landing mid-refresh win", async () => {
    ...
    instance.commit({ token: "reconnected", expiresAt: live.expiresAt });
    resolve({ token: "refreshed", expiresAt: live.expiresAt });
  ...
  it("issues an unguessable identity per commit that a deleteAll cannot reissue", () => {
    const kv = makeKv();
    const instance = coordinator(kv);
  ...
  it("retires the legacy migration when the account is cleared before its first read", () => {
    ...
    const upgrade = vi.fn((legacy: Pick<CredentialsKv, "get">) => {
      ...
      return token === undefined
        ? undefined
        : { credentials: { token, expiresAt: live.expiresAt }, legacyKeys: ["legacy-token"] };
    });
  ...
  it("cannot resurrect a legacy grant that was adopted and then cleared", () => {
    const kv = makeKv();
    kv.put("legacy-token", "legacy");
    // This upgrade reaps nothing, so the marker is the only thing retiring the legacy key.
    const upgrade = vi.fn((legacy: Pick<CredentialsKv, "get">) => {
      const token = legacy.get<string>("legacy-token");
      return token === undefined
        ? undefined
        : { credentials: { token, expiresAt: live.expiresAt }, legacyKeys: [] };
    });
    const instance = coordinator(kv, upgrade);
  ...
  it("rotates the fence even when the credential write fails", async () => {
    ...
    expect(() => instance.commit(live)).toThrow("storage unavailable");
  ...
  it("migrates legacy keys once, then reads the migrated record", () => {
    ...
    const upgrade = vi.fn((storage: Pick<CredentialsKv, "get">) => {
      ...
      return {
        credentials: { token, expiresAt: live.expiresAt },
...
describe("CredentialSource", () => {
  ...
  it("caches and coalesces account round-trips", async () => {
  ...
  it("re-fetches once the record cache goes stale", async () => {
    ...
    try {
      ...
      // The 30s window the source reuses a fetched record for has closed.
