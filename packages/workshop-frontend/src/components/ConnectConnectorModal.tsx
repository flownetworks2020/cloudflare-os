    ...
    >
      ...
      >
        <div className="shrink-0 flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          ...
          <Dialog.Close
            render={(props) => (
              <WorkshopIconButton {...props} disabled={busy} aria-label={t`Close`}>
                <X size={16} />
              </WorkshopIconButton>
            )}
          />
        ...
        <div className="new-gatekeeper-scroll-balanced min-h-0 flex-1 overflow-y-auto px-5 py-4">
            ...
            <div className="mt-5">
              <h3 className="mb-2 text-[12px] leading-4 font-semibold uppercase tracking-[0.6px] text-kumo-inactive">
                {granular
                  ? isManage
                    ? t`Resources`
                    : t`Resources to enable`
                  : t`What this gatekeeper can do`}
              </h3>
                    ...
                    >
                      ...
                      {grantable && (
                        <Switch
                          ...
                          aria-label={
                            isManage
                              ? t`Grant ${resource.title}`
                              : t`Enable ${resource.title}`
                          }
                          checked={checked}
                          disabled={disabled}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-[12px] leading-[17px] font-normal tracking-[-0.2px] text-kumo-default">
                  ...
                  <span className="text-kumo-subtle">
                    <Trans>Each Gadget only sees the resources you connect. If the workspace is shared,
                    Gatekeeper verifies other users have the required permissions before they can
                    access those resources.</Trans>
                  </span>
                </div>
              </div>
            </div>
          ...
          {isManage && (
            <div className="mt-5 rounded-lg border border-kumo-line bg-kumo-elevated px-4 py-3 text-[12px] leading-[17px] font-normal tracking-[-0.2px] text-kumo-subtle">
              <Trans>This account can be used by Gadgets you connect it to. Shared users must have the
              required permissions before they can access those connected resources.</Trans>
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 flex items-center justify-between gap-3 border-t border-kumo-line bg-kumo-base px-5 py-3">
          {isManage && confirmingDisconnect ? (
            ...
            <p className="m-0 min-w-0 flex-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              <Trans>Select at least one resource to continue.</Trans>
            </p>
          ) : (
            <span aria-hidden />
          )}
                ...
                {confirmingDisconnect ? (
                  ...
                  <>
                    ...
                    >
                      {ensuringBusy
                        ? t`Opening...`
                        : t`Continue to ${vendorDescription.displayName}`}
                    </WorkshopButton>
                  </>
                ) : (
                  <>
              ...
              <>
                ...
                >
                  {autoProvisions
                    ? connecting
                      ? t`Adding...`
                      : t`Add ${vendorDescription.displayName}`
                    : connecting
                    ? t`Opening...`
                    : t`Continue to ${vendorDescription.displayName}`}
                </WorkshopButton>
