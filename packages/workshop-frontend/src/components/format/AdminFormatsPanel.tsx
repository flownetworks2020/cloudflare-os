...
export default function AdminFormatsPanel({
  ...
  return (
    <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-6">
      <h2 className="mb-1 text-lg font-semibold text-kumo-strong"><Trans>Standard formats</Trans></h2>
      <p className="mb-5 text-sm text-kumo-subtle">
        <Trans>A promoted blueprint is offered by name (“New Doc”, “New Slides”) wherever people start
        something, and the agent is told to prefer it over building the same thing from scratch.</Trans>
      </p>
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-[11px] text-kumo-inactive">
                  {candidate.declared
                    ? t`Produces ${candidate.declared.plural}`
  ...
  return (
    <div className="mb-5 rounded-lg border border-dashed border-kumo-line bg-kumo-tint/40 p-4">
      <p className="mb-2.5 text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        <Trans>What people will see</Trans>
      </p>
      {formats.length === 0 ? (
        <p className="text-[13px] italic text-kumo-inactive">
          <Trans>Nothing yet. People will only see “New workspace”.</Trans>
        </p>
      ) : (
      ...
      <p className="mt-2.5 text-[12px] leading-4 text-kumo-subtle">
        <Trans>In the composer’s + menu, the command palette, and on an empty Outputs page, in this order.</Trans>
      </p>
    </div>
  )
}
...
function EmptyState() {
  return (
    <div className="mb-5 rounded-lg border border-kumo-line bg-kumo-base px-4 py-5 text-center">
      <p className="text-sm font-medium text-kumo-default"><Trans>No standard formats yet</Trans></p>
      <p className="mx-auto mt-1 max-w-md text-[13px] leading-[18px] text-kumo-subtle">
        <Trans>Promote a blueprint to offer it by name wherever people start something, and to have the
        agent prefer it over building the same thing from scratch.</Trans>
      </p>
    </div>
  )
}
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2">
              <span className="truncate text-sm font-medium text-kumo-default">
                {format.output ? t`New ${format.output.noun}` : format.blueprintTitle || format.blueprintId}
              </span>
              {format.bundled && <Badge><Trans>Bundled</Trans></Badge>}
              {!format.enabled && !format.missing && <Badge>Off</Badge>}
              {needsNaming && <Badge tone="warn"><Trans>Needs a name</Trans></Badge>}
            </span>
            <span className="mt-0.5 block truncate text-xs text-kumo-subtle">
              {format.missing
                ...
                : t`${format.blueprintTitle} · shown under ${format.output!.plural} on Outputs`}
            </span>
          </span>
        ...
        <div className="flex flex-col gap-4 border-t border-kumo-line px-3 py-4">
          {format.missing ? (
            <p className="text-[13px] text-kumo-subtle">
              <Trans>The blueprint behind this format was deleted, so nobody is offered it. Remove the
              entry.</Trans>
            </p>
          ) : (
            <>
              ...
              <Fieldset
                ...
                detail={t`Standard formats are listed first in the agent’s catalog, as the entry below — the blueprint’s own description does most of the work. Add a hint only if the agent needs to know when to prefer this format over another one.`}
              >
                <OverrideField
                  label={t`Hint`}
