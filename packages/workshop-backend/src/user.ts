...
import { Gatekeeper, GatekeeperUser, GatekeeperUserVerifier, GatekeeperVendor, AccountDescription, VendorDescription, GatekeeperConnectCallback, ConnectHandoff, SupportedResource, ResourceConfiguratorFrame, AppUiContext, GatekeeperUiFrame, managedAiModelId } from "@gadgets/workshop-shared/gatekeeper";
  ...
  *#connectedAccountRecords(): Generator<ConnectedAccountRecord> {
  ...
  async completeConnectHandoff(ticket: string): Promise<void> {
    ...
    if (/^[0-9a-f]{64}$/.test(ticket)) {
      let ticketHash = await hashSecret(Uint8Array.fromHex(ticket));
